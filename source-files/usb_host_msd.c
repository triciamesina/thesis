                ...
                case STATE_INITIALIZE_DEVICE:
                    switch (deviceInfoMSD[i].state & SUBSTATE_MASK)
                    {
                        ...
                        case SUBSTATE_GET_MAX_LUN_COMPLETE:
                            ...
                            if (USBHostMSDSCSIInitialize( i, deviceInfoMSD[i].deviceAddress, 0, 0 ))
                            {
                                USBHostMSDSCSIEventHandler( i, deviceInfoMSD[i].deviceAddress, EVENT_MSD_MAX_LUN, &(deviceInfoMSD[i].maxLUN), 1 );
    ...
    switch (event)
    {
        ...
        case EVENT_DETACH:           // USB cable has been detached (data: BYTE, address of device)
            ...
            if (i < USB_MAX_MASS_STORAGE_DEVICES)
            {
                ...
                USBHostMSDSCSIEventHandler( i, address, EVENT_DETACH, NULL, 0 );
            }
            return TRUE;
            break;
...
void _USBHostMSD_ResetStateJump( BYTE i )
{
    ...
    {
        USBHostMSDSCSIEventHandler( i, deviceInfoMSD[i].deviceAddress, EVENT_MSD_RESET, NULL, 0 );
