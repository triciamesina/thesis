...
allow the File System code tao reference the functions in this file.
...
#define USB_MAX_DRIVES              4           // Max number of USB MSD devices
...
BOOL USBHostMSDSCSIInitialize( BYTE driveNumber, BYTE address, DWORD flags, BYTE clientDriverID )
{
    #ifdef DEBUG_MODE
    ...
    if (deviceAddress[driveNumber] == 0)
    {
        ...
        deviceAddress[driveNumber] = address;
        return TRUE;
    }
    else
    {
...
BOOL USBHostMSDSCSIEventHandler( BYTE driveNumber, BYTE address, USB_EVENT event, void *data, DWORD size )
{
    if (deviceAddress[driveNumber] == address)
    {
        switch( event )
        {
            case EVENT_MSD_NONE:
            ...
            case EVENT_MSD_MAX_LUN:
                ...
                #endif
                mediaInformation[driveNumber].maxLUN                     = *((BYTE *)data);
                mediaInformation[driveNumber].validityFlags.bits.maxLUN  = 1;
                return TRUE;
                break;
            ...
            case EVENT_DETACH:                      // USB cable has been detached (data: BYTE, address of device)
                ...
                #endif
                deviceAddress[driveNumber]                           = 0;
                mediaInformation[driveNumber].validityFlags.value    = 0;
                return TRUE;
                break;
...
/****************************************************************************
  ...
  Description:
    This function determines if a mass storage device in port 1 is attached and ready
    to use.
...
/****************************************************************************
  ...
  ***************************************************************************/
// MODIFICATION 1/13/2016       FOR PORT 1
MEDIA_INFORMATION * USBHostMSDSCSIMediaInitialize( BYTE driveNumber )
{
    BYTE        attempts;
    DWORD       byteCount;
    ...
    if (deviceAddress[driveNumber] == 0)
    {
        mediaInformation[driveNumber].errorCode = MEDIA_DEVICE_NOT_PRESENT;
        return &mediaInformation[driveNumber];
    }
    ...
    while (attempts != 0)
    {
        ...
        errorCode = USBHostMSDRead( deviceAddress[driveNumber], 0, commandBlock, 10, inquiryData, 8 );
        #ifdef DEBUG_MODE
        ...
        if (!errorCode)
        {
            ...
            // Determine sector size.
                ...
                UART2PrintString( "\r\n" );
            #endif
            mediaInformation[driveNumber].sectorSize                     = (inquiryData[7] << 12) + (inquiryData[6] << 8) + (inquiryData[5] << 4) + (inquiryData[4]);
            mediaInformation[driveNumber].validityFlags.bits.sectorSize  = 1;

            mediaInformation[driveNumber].errorCode = MEDIA_NO_ERROR;
            return &mediaInformation[driveNumber];
        }
        else
        {
            ...
            errorCode = USBHostMSDRead( deviceAddress[driveNumber], 0, commandBlock, 6, inquiryData, 18 );
            #ifdef DEBUG_MODE
                UART2PutHex( errorCode ) ;
                UART2PutChar( ' ' );
            #endif
    ...
    mediaInformation[driveNumber].errorCode = MEDIA_CANNOT_INITIALIZE;
    return &mediaInformation[driveNumber];
...
BYTE USBHostMSDSCSIMediaReset( BYTE driveNumber  )
{
    DWORD   byteCount;
    BYTE    errorCode;

    errorCode = USBHostMSDResetDevice( deviceAddress[driveNumber] );
    if (errorCode)
    {
        return errorCode;
    }
    ...
    if (USBHostMSDTransferIsComplete( deviceAddress[driveNumber], &errorCode, &byteCount ))
    {
        return errorCode;
    }
...
BYTE USBHostMSDSCSISectorRead( BYTE driveNumber, DWORD sectorAddress, BYTE *dataBuffer )
{
    DWORD   byteCount;
    BYTE    commandBlock[10];
    BYTE    errorCode;
    ...
    errorCode = USBHostMSDRead( deviceAddress[driveNumber], 0, commandBlock, 10, dataBuffer, mediaInformation[driveNumber].sectorSize );
    #ifdef DEBUG_MODE
...
BYTE USBHostMSDSCSISectorWrite( BYTE driveNumber, DWORD sectorAddress, BYTE *dataBuffer, BYTE allowWriteToZero )
{
    DWORD   byteCount;
    BYTE    commandBlock[10];
    BYTE    errorCode;
    ...
    // Currently using LUN=0.  When the File System supports multiple LUN's, this will change.
    errorCode = USBHostMSDWrite( deviceAddress[driveNumber], 0, commandBlock, 10, dataBuffer, mediaInformation[driveNumber].sectorSize );
    #ifdef DEBUG_MODE
