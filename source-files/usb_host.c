...
#define USB_HUB_SUPPORT_INCLUDED

// This value represents the maximum number of attached devices to the usb host.
// More than 1 device can be made possible with hub.
...
static BYTE                         *pEP0Data;  // A data buffer for use by EP0.
...
typedef struct _USB_DEVICE_LIST
{
	...
	BYTE							*pCurrentConfigurationDescriptor; //*   //= NULL;	// Pointer to the current configuration descriptor of the attached device.
	BYTE							*pDeviceDescriptor;     //*             //= NULL;	// Pointer to the Device Descriptor of the attached device.
	volatile WORD					state;		//*									// State machine state of the attached device.
	volatile WORD					usbOverrideHostState; //*							// Next state machine state, when set by interrupt processing.
...
static USB_DEVICE_LIST				portdeviceInfo[USB_MAX_DEVICES];
...
//	DBPRINTF("USBHostInit()\n");
    ...
    if (portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0 == NULL)
    {
        ...
        {
            #ifdef DEBUG_MODE
    //            DBPRINTF( "HOST: Cannot allocate for endpoint 0.\r\n" );
            #endif
            //return USB_MEMORY_ALLOCATION_ERROR;
            return FALSE;
        }
	//	DBPRINTF("Allocated for Endpoint 0. DeviceNumber = %x\n", DeviceNumber);
    ...
    usbDeviceInfo.pInterfaceList = 0;
	// Initialize other variables.
    if (DeviceNumber == 0) {
		while (DeviceNumber < USB_MAX_DEVICES) {
			portdeviceInfo[DeviceNumber].pCurrentEndpoint                   = NULL;
		    portdeviceInfo[DeviceNumber].state                      = STATE_DETACHED;
		    portdeviceInfo[DeviceNumber].usbOverrideHostState             = NO_STATE;
		    portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddressAndSpeed = 0;
		    portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddress        = 0;
		break;
		}
	}
    countConfigurations = 0;              // Count the Configuration Descriptors read during enumeration.
    numCommandTries = 0;                  // The number of times the current command has been tried.
    numEnumerationTries = 0;              // The number of times enumeration has been attempted on the attached device.
    numTimerInterrupts = 0;               // The number of milliseconds elapsed during the current waiting period.
    pCurrentEndpoint = NULL;                 // Pointer to the endpoint currently performing a transfer.
    pCurrentConfigurationDescriptor = NULL;  // Pointer to the current configuration descriptor of the attached device.
    pDeviceDescriptor = NULL;                // Pointer to the Device Descriptor of the attached device.
            ...
            BYTE clientDriverID )
{
	...
	DeviceNumber = _USB_FindDeviceAddress( deviceAddress );
      if (DeviceNumber == USB_MAX_DEVICES)
    {
        return USB_UNKNOWN_DEVICE;
    }
...
BYTE USBHostResumeDevice( BYTE deviceAddress )
{
    ...
    if (portdeviceInfo[DeviceNumber].state != (STATE_RUNNING | SUBSTATE_SUSPEND_AND_RESUME | SUBSUBSTATE_SUSPEND))
    {
        return USB_ILLEGAL_REQUEST;
    }
...
/****************************************************************************
  Function:
    void USBHostTasks( BYTE DeviceNumber )
...

{
    ...
    #ifdef DEBUG_MODE
//        UART2PutChar('<');
//        UART2PutHex( portdeviceInfo[DeviceNumber].state>>8 );
//        UART2PutHex( portdeviceInfo[DeviceNumber].state & 0xff );
//        UART2PutChar('-');
...
//	DBPRINTF("USBHostTasks Start\n");
    ...
    // If the ATTACH bit is 0, we have detached.
            ...
            if (((portdeviceInfo[DeviceNumber].state & STATE_MASK) != STATE_DETACHED) && !U1IRbits.ATTACHIF)
            {
                #ifdef DEBUG_MODE
                    UART2PutChar( '>' );
                    UART2PutChar( ']' );
        ...
        while (StructQueueIsNotEmpty(&usbEventQueue, USB_EVENT_QUEUE_DEPTH))
        {
            ...
            switch(item->event)
            {
                ...
                case EVENT_BUS_ERROR:
                    _USB_NotifyClients( portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddress, item->event, &item->TransferData, sizeof(HOST_TRANSFER_DATA) );
                    break;
                default:
                    break;
            }
    ...
    if (portdeviceInfo[DeviceNumber].usbOverrideHostState != NO_STATE)
    {
        #ifdef DEBUG_MODE
            UART2PutChar('>');
        #endif
        portdeviceInfo[DeviceNumber].state = portdeviceInfo[DeviceNumber].usbOverrideHostState;
        portdeviceInfo[DeviceNumber].usbOverrideHostState = NO_STATE;
    }
...
// 	DBPRINTF("\tstate %x\n",portdeviceInfo[DeviceNumber].state);
    ...
    {
        case STATE_DETACHED:
            switch (portdeviceInfo[DeviceNumber].state & SUBSTATE_MASK)
            {
                case SUBSTATE_INITIALIZE:
                    ...
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->next                         = NULL;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->status.val                   = 0x00;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->status.bfUseDTS              = 1;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->status.bfTransferComplete    = 1;    // Initialize to success to allow preprocessing loops.
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->status.bfNAKTimeoutEnabled   = 1;    // So we can catch devices that NAK forever during enumeration
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->timeoutNAKs                  = USB_NUM_CONTROL_NAKS;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->wMaxPacketSize               = 64;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->dataCount                    = 0;    // Initialize to 0 since we set bfTransferComplete.
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->bEndpointAddress             = 0;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->transferState                = TSTATE_IDLE;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->bmAttributes.bfTransferType  = USB_TRANSFER_TYPE_CONTROL;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pEndpoint0->clientDriver                 = CLIENT_DRIVER_HOST;
                    // Initialize any device specific information.
					portdeviceInfo[DeviceNumber].numEnumerationTries = USB_NUM_ENUMERATION_TRIES;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.currentConfiguration  = 0; // Will be overwritten by config process or the user later
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.attributesOTG         = 0;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddressAndSpeed = 0;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.val             = 0;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList        = NULL;
                    
					if(DeviceNumber != 0)	// If the device is not directly connected to the root hub
					{
						...
						portdeviceInfo[DeviceNumber].usbDeviceInfo.attributesOTG             = 0;
						portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.val                 = 0;
						portdeviceInfo[DeviceNumber].state = STATE_ATTACHED | SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE;
                ...
                case SUBSTATE_TURN_ON_POWER:
                    ...
                    else
                    {
                        ...
                        portdeviceInfo[DeviceNumber].state = STATE_DETACHED | SUBSTATE_WAIT_FOR_POWER;
                    }
                    break;
...
// 							DBPRINTF("Wait for Settling\t numTimerInterrupts = %x\n", numTimerInterrupts);
                ...
                case SUBSTATE_RESET_DEVICE:
                    ...
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SET_RESET:
                            ...
                            U1CONbits.PPBRST                    = 1;
                            U1CONbits.PPBRST                    = 0;
                            portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfPingPongIn    = 0;
                            portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfPingPongOut   = 0;
                ...
                case SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE:
                    ...
                    {	
                        ...
                        case SUBSUBSTATE_WAIT_FOR_GET_DEVICE_DESCRIPTOR_SIZE:
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                ...
                                else
                                {
                                    ...
                                    // enumerate again.
                                    _USB_CheckCommandAndEnumerationAttempts(DeviceNumber);
                                }
                            }
                            break;
                        ...
                        case SUBSUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE_COMPLETE:
                            ...
                            portdeviceInfo[DeviceNumber].pDeviceDescriptor[0] = pEP0Data[0];
							pDeviceDescriptor[0] = portdeviceInfo[DeviceNumber].pDeviceDescriptor[0];
                ...
                case SUBSTATE_GET_DEVICE_DESCRIPTOR:
                    ...
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_GET_DEVICE_DESCRIPTOR:
                            ...
                            pEP0Data[4] = 0;
                            pEP0Data[5] = 0;
                            pEP0Data[6] = pDeviceDescriptor[0];
                            pEP0Data[7] = 0;
                            _USB_InitControlRead( usbDeviceInfo.pEndpoint0, pEP0Data, 8, pDeviceDescriptor, pDeviceDescriptor[0]  );
                            _USB_Port_SetNextSubSubState(DeviceNumber);
                            break;
                ...
                case SUBSTATE_VALIDATE_VID_PID:
                    ...
                    _USB_Port_SetNextState(DeviceNumber);
                    break;
            }
            break;
                ...
                case SUBSTATE_SET_DEVICE_ADDRESS:
                    ...
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        ...
                        case SUBSUBSTATE_SET_DEVICE_ADDRESS_COMPLETE:
                            ...
                            usbDeviceInfo.deviceAddressAndSpeed = (portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfIsLowSpeed << 7) | portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddress;
							portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddressAndSpeed = usbDeviceInfo.deviceAddressAndSpeed;
		...
		//					DBPRINTF("pTemp = %x\tsizeof(USB_CONFIGURATION) = %i\n", pTemp, sizeof(USB_CONFIGURATION));
                            ...
                            portdeviceInfo[DeviceNumber].pCurrentConfigurationDescriptor            = ((USB_CONFIGURATION *)pTemp)->descriptor;
							pCurrentConfigurationDescriptor = portdeviceInfo[DeviceNumber].pCurrentConfigurationDescriptor;
                            ((USB_CONFIGURATION *)pTemp)->configNumber = portdeviceInfo[DeviceNumber].countConfigurations;
                ...
                case SUBSTATE_GET_CONFIG_DESCRIPTOR:
                    ...
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        ...
                        case SUBSUBSTATE_GET_CONFIG_DESCRIPTOR_COMPLETE:
                            ...
                            _USB_InitErrorCounters();
                            portdeviceInfo[DeviceNumber].countConfigurations --;
                            if (portdeviceInfo[DeviceNumber].countConfigurations)
                            {
                                // There are more descriptors that we need to get.
                                portdeviceInfo[DeviceNumber].state = STATE_CONFIGURING | SUBSTATE_GET_CONFIG_DESCRIPTOR_SIZE;
                            }
                            else
                            {
                ...
                case SUBSTATE_SELECT_CONFIGURATION:
                    ...
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SELECT_CONFIGURATION:
                            ...
                            if (portdeviceInfo[DeviceNumber].usbDeviceInfo.currentConfiguration == 0)
                            {
                                // Search for a supported class-specific configuration.
                                pCurrentConfigurationNode = portdeviceInfo[DeviceNumber].usbDeviceInfo.pConfigurationDescriptorList;
                                while (pCurrentConfigurationNode)
                                {
                                    pCurrentConfigurationDescriptor = pCurrentConfigurationNode->descriptor;
									portdeviceInfo[DeviceNumber].pCurrentConfigurationDescriptor = pCurrentConfigurationDescriptor;
                                    if (_USB_ParseConfigurationDescriptor(DeviceNumber))
                                    {
                                        break;
                                    }
                                    else
                            ...
                            else
                            {
                                ...
                                pCurrentConfigurationNode = portdeviceInfo[DeviceNumber].usbDeviceInfo.pConfigurationDescriptorList;
                                while (pCurrentConfigurationNode && pCurrentConfigurationNode->configNumber != portdeviceInfo[DeviceNumber].usbDeviceInfo.currentConfiguration)
                                {
                                    pCurrentConfigurationNode = pCurrentConfigurationNode->next;
                                }
                                pCurrentConfigurationDescriptor = pCurrentConfigurationNode->descriptor;
                            ...
                            //If No OTG Then
                            if (portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfConfiguredOTG)
                            {
                                // Did we fail to configure?
                                if (pCurrentConfigurationNode == NULL)
                                {
                                    // Failed to find a supported configuration.
                                    _USB_Port_SetErrorCode( DeviceNumber, USB_HOLDING_UNSUPPORTED_DEVICE );
                                    _USB_Port_SetHoldState(DeviceNumber);
                                }
                                else
                                {
                        ...
                        case SUBSUBSTATE_WAIT_FOR_SET_OTG_DONE:
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                ...
                                {
                                    #ifdef  USB_SUPPORT_OTG
                                        if (portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfAllowHNP)
                                        {
                                            USBOTGEnableHnp();
                                        }
                                     #endif
                    ...
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        ...
                        case SUBSUBSTATE_INIT_CLIENT_DRIVERS:
                            ...
                            if (portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfUseDeviceClientDriver)
                            {
                                ...
                                //#ifdef DEBUG_MODE
                                //    DBPRINTF( "HOST: Using device client driver.\r\n" );
                                //#endif
                                temp = portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceClientDriver;
                                if (!usbClientDrvTable[temp].Initialize(portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddress, usbClientDrvTable[temp].flags, temp))
                                {
                                    _USB_Port_SetErrorCode( DeviceNumber, USB_HOLDING_CLIENT_INIT_ERROR );
                                    _USB_Port_SetHoldState(DeviceNumber);
                                }
                            }
                            else
                            {
                                ...
                                //#ifdef DEBUG_MODE
                                //    DBPRINTF( "HOST: Scanning interfaces.\r\n" );
                                //#endif
                                pCurrentInterface = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList;
                ...
                case SUBSTATE_SUSPEND_AND_RESUME:
                    switch (portdeviceInfo[DeviceNumber].state & SUBSUBSTATE_MASK)
                    {
                        ...
                        case SUBSUBSTATE_RESUME_COMPLETE:
                            ...
                            portdeviceInfo[DeviceNumber].state = STATE_RUNNING | SUBSTATE_NORMAL_RUN;
                            break;
                    }
            }
            break;
...
//					DBPRINTF("\nError is %x\n",portdeviceInfo[DeviceNumber].usbDeviceInfo.errorCode);
                    switch ( portdeviceInfo[DeviceNumber].usbDeviceInfo.errorCode )
                    {
...
//                            temp = EVENT_HUB_ATTACH;
                    ...
                    USB_HOST_APP_EVENT_HANDLER( portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddress, temp, &portdeviceInfo[DeviceNumber].usbDeviceInfo.currentConfigurationPower , 1 );
...
void _USB_CheckCommandAndEnumerationAttempts( BYTE DeviceNumber )
{
    #ifdef DEBUG_MODE
        UART2PutChar( '=' );
    #endif
    ...
    if (numCommandTries != 0)
    {
        ...
        portdeviceInfo[DeviceNumber].state &= ~SUBSUBSTATE_MASK;
    }
    else
    {
        ...
        if (numEnumerationTries != 0)
        {
            ...
            portdeviceInfo[DeviceNumber].state = STATE_ATTACHED | SUBSTATE_RESET_DEVICE;
        }
        else
        {
            // Give up.  The device is not responding properly.
            _USB_Port_SetErrorCode( DeviceNumber, USB_CANNOT_ENUMERATE );
            _USB_Port_SetHoldState(DeviceNumber);
        }
    }
}
...
/****************************************************************************
  Function:
    BYTE _USB_FindDeviceAddress( BYTE deviceAddress )

  Description:
    This function searches the list of devices to try to find the device
    matching the device address.

  Precondition:
    None

  Parameters:
    BYTE deviceAddress   - The address of the device

  Returns:
    TRUE	-	A device matches the address.
	FALSE	-	No device matches the address.
...
BYTE _USB_FindDeviceAddress( BYTE deviceAddress )
{
    ...
    while (DeviceNumber < USB_MAX_DEVICES)
    {
		if (deviceAddress == portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddress)
		{
			pDeviceDescriptor = portdeviceInfo[DeviceNumber].pDeviceDescriptor;
 			pCurrentConfigurationDescriptor = portdeviceInfo[DeviceNumber].pCurrentConfigurationDescriptor;
			usbDeviceInfo.deviceAddressAndSpeed = portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceAddressAndSpeed;
			usbDeviceInfo.pInterfaceList = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList;
            // We have found the address as a hub attached device.
			// We now switch the parameters to the attached device.
			return DeviceNumber;
		}
		DeviceNumber++;
    }
    return USB_MAX_DEVICES;			// return USB_MAX_DEVICES indicating that the device is not found
}
...
void _USB_FreeConfigMemory( BYTE DeviceNumber )
{
    USB_INTERFACE_INFO          *pTempInterface;
    USB_INTERFACE_SETTING_INFO  *pTempSetting;
    USB_ENDPOINT_INFO           *pTempEndpoint;

    while (portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList != NULL)
    {
        pTempInterface = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->next;

        while (portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings != NULL)
        {
            pTempSetting = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings->next;

            while (portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings->pEndpointList != NULL)
            {
                pTempEndpoint = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings->pEndpointList->next;
                free( (BYTE *)portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings->pEndpointList );
                portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings->pEndpointList = pTempEndpoint;
            }
            free( (BYTE *)portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings );
            portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList->pInterfaceSettings = pTempSetting;
        }
...
/****************************************************************************
  Function:
    BOOL _USB_ParseConfigurationDescriptor( BYTE DeviceNumber )
...
BOOL _USB_ParseConfigurationDescriptor( BYTE DeviceNumber )
{
    ...
    portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfSupportsOTG   = 0;
    portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfConfiguredOTG = 1;
    ...
    if (!USB_HOST_APP_EVENT_HANDLER( USB_ROOT_HUB, EVENT_VBUS_REQUEST_POWER,
            &powerRequest, sizeof(USB_VBUS_POWER_EVENT_DATA) ))
    {
        portdeviceInfo[DeviceNumber].usbDeviceInfo.errorCode = USB_ERROR_INSUFFICIENT_POWER;
        return FALSE;
    }
    ...
    while (index < wTotalLength)
    {
        ...
        else
        {
            ...
            if (portdeviceInfo[DeviceNumber].usbDeviceInfo.flags.bfUseDeviceClientDriver)
            {
                ClientDriver = portdeviceInfo[DeviceNumber].usbDeviceInfo.deviceClientDriver;
            }
            else
            {
...

            {
                ...
                newInterfaceInfo = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList;
                ...
                if (newInterfaceInfo == NULL)
                {
                    ...
                    newInterfaceInfo->next                  = portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList;
                    portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList = newInterfaceInfo;
                }
...
//    }
    ...
    if (portdeviceInfo[DeviceNumber].usbDeviceInfo.pInterfaceList == NULL)
    {
    ...
    // Set configuration.
    portdeviceInfo[DeviceNumber].usbDeviceInfo.currentConfiguration      = currentConfiguration;
...
#endif
{
	...
	BYTE DeviceNumber;
	unsigned long endtime, startcount;
    #if defined( __C30__)
        IFS5 &= 0xFFBF;
    #elif defined( __PIC32MX__)
        IFS1CLR = 0x02000000;
    ...
    if (U1IEbits.ATTACHIE && U1IRbits.ATTACHIF)
    {
        ...
        if (portdeviceInfo[0].state == (STATE_DETACHED | SUBSTATE_WAIT_FOR_DEVICE))
        {
            portdeviceInfo[0].usbOverrideHostState = STATE_ATTACHED;
        }
    ...
    if (U1IEbits.DETACHIE && U1IRbits.DETACHIF)
    {
            ...
            UART2PutChar( ']' );
        #endif
		DeviceNumber = CurrentPort - 1;
        U1IR                    = USB_INTERRUPT_DETACH;
        U1IEbits.DETACHIE       = 0;
        portdeviceInfo[DeviceNumber].usbOverrideHostState    = STATE_DETACHED;
...
void _USB_Port_SetErrorCode( BYTE DeviceNumber, BYTE errorCode ) {
	portdeviceInfo[DeviceNumber].usbDeviceInfo.errorCode = errorCode;
}
void _USB_Port_SetHoldState( BYTE DeviceNumber ) {
	portdeviceInfo[DeviceNumber].state = STATE_HOLDING;
}

void  _USB_Port_SetNextState( BYTE DeviceNumber ) {
	portdeviceInfo[DeviceNumber].state = (portdeviceInfo[DeviceNumber].state & STATE_MASK) + NEXT_STATE;
}

void  _USB_Port_SetNextSubState( BYTE DeviceNumber ) {
	portdeviceInfo[DeviceNumber].state = (portdeviceInfo[DeviceNumber].state & (STATE_MASK | SUBSTATE_MASK)) + NEXT_SUBSTATE;
}
...
BYTE USBHostDeviceDetach ( BYTE DevAddr ) {
	...
	DeviceNumber = _USB_FindDeviceAddress( DevAddr );
      if (DeviceNumber == USB_MAX_DEVICES) {
        return USB_UNKNOWN_DEVICE;
   		}
